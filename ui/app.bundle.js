...
var UnknownNodeError = class extends Error {
};
var InvalidAnswerError = class extends Error {
};
function findNode(flow2, id) {
  const node = flow2.nodes.find((n) => n.id === id);
  if (!node) throw new UnknownNodeError(`Knoten "${id}" nicht im Flow-Graph gefunden`);
  return node;
}
function pickLocalized(value, lang = "de") {
  return value[lang] ?? value.de;
}
function emphasisFor(slider, thresholds) {
...
function submitAnswer(session, answer) {
  ...
  switch (node.type) {
    ...
    case "gatekeeper":
      throw new InvalidAnswerError("gatekeeper ist terminal, keine weitere Antwort m\xF6glich");
    case "purpose_reselect":
      return next(node.next);
    case "trust_question":
    ...
    case "review":
      throw new InvalidAnswerError("review ist terminal, keine weitere Antwort m\xF6glich");
    default:
      throw new UnknownNodeError(`Unbekannter Knotentyp: ${node.type}`);
  }
}
...
var LocalizedSchema = external_exports.object({
  de: external_exports.string(),
  fr: external_exports.string().optional(),
  it: external_exports.string().optional(),
  en: external_exports.string().optional()
});
var ConversionTypeSchema = external_exports.enum([
  "call",
  "whatsapp",
  "contact_form",
  "inquiry_project",
...
var ConversionClassSchema = external_exports.enum(["action", "trust", "mixed"]);
var ProofTypeSchema = external_exports.enum([
  "qualification",
  "certification",
  "experience_years",
  "specialization",
...
var BlockIdSchema = external_exports.enum([
  "hero",
  "services",
  "pricing",
  "about_me",
...
var SemiDynamicModuleSchema = external_exports.enum([
  "promotion",
  "news_notice",
  "job_opening",
  "holiday_hours",
...
var RegulatoryFactSchema = external_exports.object({
...
var BranchProfileSchema = external_exports.object({
  ...
  branch_aliases: external_exports.array(external_exports.string()).optional(),
  ...
  default_blocks: external_exports.array(BlockIdSchema).min(1),
...
// src/schema/flow.ts
var NodeIdSchema = external_exports.string().min(1);
var BranchSelectNodeSchema = external_exports.object({
  id: NodeIdSchema,
  type: external_exports.literal("branch_select"),
  next: NodeIdSchema
});
var ConfirmDefaultNodeSchema = external_exports.object({
  id: NodeIdSchema,
  type: external_exports.literal("confirm_default"),
  prompt_template: external_exports.literal("default_purpose"),
  on_exact: NodeIdSchema,
  on_more: NodeIdSchema,
  on_no: NodeIdSchema
});
var ChecklistNodeSchema = external_exports.object({
  id: NodeIdSchema,
  type: external_exports.literal("checklist"),
  source_field: external_exports.literal("optional_blocks"),
  next: NodeIdSchema
});
var LookupGateNodeSchema = external_exports.object({
  id: NodeIdSchema,
  type: external_exports.literal("lookup_gate"),
  generic_options: external_exports.array(LocalizedSchema).min(1),
  signal_field: external_exports.literal("out_of_scope_signals"),
  on_match: NodeIdSchema,
  on_no_match: NodeIdSchema
});
var GatekeeperNodeSchema = external_exports.object({
  id: NodeIdSchema,
  type: external_exports.literal("gatekeeper"),
  message: LocalizedSchema,
  terminal: external_exports.literal(true)
});
var PurposeReselectNodeSchema = external_exports.object({
  id: NodeIdSchema,
  type: external_exports.literal("purpose_reselect"),
  next: NodeIdSchema
});
var TrustQuestionNodeSchema = external_exports.object({
  id: NodeIdSchema,
  type: external_exports.literal("trust_question"),
  next: NodeIdSchema
});
var DepthConfigSchema = external_exports.object({
  depth: external_exports.enum(["full", "short"]),
  branching: external_exports.boolean(),
  visible: external_exports.literal(true),
  max_options: external_exports.number().int().positive().optional()
});
var ScalableQuestionNodeSchema = external_exports.object({
  id: NodeIdSchema,
  type: external_exports.literal("scalable_question"),
  depth_source: external_exports.literal("conversion_class"),
  depth_table: external_exports.record(ConversionClassSchema, DepthConfigSchema),
  next: NodeIdSchema
});
var EmphasisThresholdSchema = external_exports.object({
  max: external_exports.number().int().min(0).max(100),
  emphasis: external_exports.enum(["full", "reduced"])
});
var CharacterQuestionNodeSchema = external_exports.object({
  id: NodeIdSchema,
  type: external_exports.literal("character_question"),
  slider_source: external_exports.literal("craft_slider"),
  emphasis_thresholds: external_exports.array(EmphasisThresholdSchema).min(1),
  next: NodeIdSchema
});
var FactsFormNodeSchema = external_exports.object({
  id: NodeIdSchema,
  type: external_exports.literal("facts_form"),
  fields: external_exports.array(external_exports.string()).min(1),
  next: NodeIdSchema
});
var MandatoryAboveThresholdSchema = external_exports.object({
  threshold: external_exports.number().int().min(0).max(100),
  block: BlockIdSchema
});
var MaterialCollectNodeSchema = external_exports.object({
  id: NodeIdSchema,
  type: external_exports.literal("material_collect"),
  slider_source: external_exports.literal("craft_slider"),
  mandatory_above_threshold: external_exports.array(MandatoryAboveThresholdSchema),
  next: NodeIdSchema
});
var ReviewNodeSchema = external_exports.object({
  id: NodeIdSchema,
  type: external_exports.literal("review"),
  terminal: external_exports.literal(true)
});
var FlowNodeSchema = external_exports.discriminatedUnion("type", [
...
var FlowGraphSchema = external_exports.object({
  entry: NodeIdSchema,
  nodes: external_exports.array(FlowNodeSchema).min(1)
});

// src/flow/flow-graph.json
var flow_graph_default = {
  entry: "F0_branch_select",
  nodes: [
    {
      id: "F0_branch_select",
      type: "branch_select",
    ...
    },
    {
      ...
      prompt_template: "default_purpose",
      on_exact: "F_vertrauen",
      on_more: "F1b_checklist",
      on_no: "F1_NEIN_gate"
    },
    {
      id: "F1b_checklist",
      type: "checklist",
      source_field: "optional_blocks",
      next: "F_vertrauen"
    },
    {
      id: "F1_NEIN_gate",
      type: "lookup_gate",
      generic_options: [
        { de: "Nur Online-Pr\xE4senz/Legitimation, keine aktive Akquise" },
        { de: "Anfrage statt direktem Erstgespr\xE4ch" }
      ],
      signal_field: "out_of_scope_signals",
      on_match: "F1_NEIN_gatekeeper",
      on_no_match: "F1_NEIN_purpose_reselect"
    },
    {
      id: "F1_NEIN_gatekeeper",
      type: "gatekeeper",
      message: {
        de: "Daf\xFCr ist der Fragebogen noch nicht gebaut. Wir merken Ihr Anliegen vor und melden uns, sobald es unterst\xFCtzt wird."
      },
      terminal: true
    },
    {
      id: "F1_NEIN_purpose_reselect",
      type: "purpose_reselect",
      next: "F_vertrauen"
    },
    {
      id: "F_vertrauen",
    ...
    {
      id: "F_conversion",
      type: "scalable_question",
      depth_source: "conversion_class",
      depth_table: {
        action: { depth: "full", branching: true, visible: true },
        trust: { depth: "short", branching: false, visible: true, max_options: 3 },
        mixed: { depth: "full", branching: true, visible: true }
      },
      next: "F_character"
    },
    {
      ...
      slider_source: "craft_slider",
      emphasis_thresholds: [
        { max: 60, emphasis: "full" },
        { max: 100, emphasis: "reduced" }
      ],
      next: "F_identity"
    },
    {
      id: "F_identity",
      type: "facts_form",
      fields: ["name", "ort", "erreichbarkeit", "sprache"],
      next: "F_material"
    },
    {
      id: "F_material",
      ...
      mandatory_above_threshold: [
        { threshold: 80, block: "gallery_portfolio" }
      ],
      next: "F_review"
    },
    {
...
// src/profiles/coiffeur.json
var coiffeur_default = {
  branch_id: "coiffeur",
  branch_label: { de: "Coiffeur / Friseur" },
  branch_aliases: ["Friseur", "Hairstylist", "Salon", "Barbier"],
  default_purpose: {
    de: "...dass neue Kunden Sie online finden und einfach einen Termin oder Kontakt aufnehmen k\xF6nnen"
  },
...
function render() {
  ...
  switch (model.type) {
    case "branch_select": {
      ...
      app.append(el("button", { onclick: () => advance(void 0) }, ["Weiter"]));
      break;
    }
    case "confirm_default": {
      app.append(el("h2", {}, [model.purpose]));
      for (const choice of model.choices) {
        const labels = { exact: "Ja, genau", more: "Ja, aber mehr", no: "Nein" };
        app.append(el("button", { onclick: () => advance(choice) }, [labels[choice] ?? choice]));
      }
      break;
    }
    case "checklist": {
      app.append(el("h2", {}, ["Zus\xE4tzliche Bausteine?"]));
      const checkboxes = [];
    ...
    case "lookup_gate": {
      ...
      for (const opt of model.generic_options) {
        app.append(el("button", { onclick: () => advance({ kind: "generic", value: opt }) }, [opt]));
      }
      for (const sig of model.signal_options) {
    ...
    case "gatekeeper": {
      ...
      app.append(el("p", {}, [model.message]));
      break;
    }
    case "purpose_reselect": {
      app.append(el("p", {}, [model.note]));
      app.append(el("button", { onclick: () => advance(void 0) }, ["Weiter"]));
      break;
    }
    case "trust_question": {
      app.append(el("h2", {}, ["Womit \xFCberzeugen Sie?"]));
      app.append(el("p", {}, [`Beleg-Typen: ${model.proof_types.join(", ")}`]));
      app.append(el("p", {}, [`Spezialisierung relevant: ${model.specialization_relevant}`]));
      const facts = model.regulatory_facts;
      if (facts.length === 0) {
    ...
    case "scalable_question": {
      ...
      if (model.branching) app.append(el("p", { className: "muted" }, ["(volle Tiefe: w\xFCrde hier in eine Tool-Folgefrage verzweigen)"]));
      break;
    }
    case "character_question": {
      app.append(el("h2", {}, [`Was soll der Besucher sp\xFCren? (Emphase: ${model.emphasis})`]));
      const sets = model.character_set;
      sets.forEach((c, i) => {
    ...
    case "facts_form": {
      ...
      for (const field of model.fields) {
        const input = el("input", { placeholder: field });
        inputs[field] = input;
    ...
    case "material_collect": {
      ...
      for (const b of blocks) {
        const select = el("select", {}, [
          ...
          ...b.mandatory ? [] : [el("option", { value: "spaeter" }, ["sp\xE4ter"]), el("option", { value: "macht_ihr" }, ["macht ihr"])]
        ]);
